...
React single-page app in `frontend/` is the user interface: add a task, edit a
task through an inline form, and delete a task after confirming.
...
is always visible on the next read (`backend/todo_list/views.py`).
...
browser (session authentication) - `frontend/src/api.js` sends it for you.
...
`docker-entrypoint.sh`) in Docker. `todo_list/admin.py` configures a
...
│   │   ├── urls.py                <- admin + includes the app's urls
│   │   ├── asgi.py
│   │   └── wsgi.py
